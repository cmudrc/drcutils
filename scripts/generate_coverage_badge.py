"""Generate coverage SVG badge from pytest-cov JSON report."""
...
def main() -> None:
    """Generate and write the coverage SVG badge."""
    payload = json.loads(COVERAGE_JSON.read_text(encoding="utf-8"))
    totals = payload.get("totals", {})
    raw = totals.get("percent_covered_display", totals.get("percent_covered", 0))
    percent = int(float(str(raw).rstrip("%")))
